...
class Floor:
    ...
    def pickup_passengers(self, flag, maximum_count, *, caller_id=None) -> Iterable[Passenger]:
        ...
        while counter < maximum_count and current is not None:
            nxt = current.next
            if current.value.has_flag(flag):
                yield self.queue.remove(current)
                counter += 1
            current = nxt
        self.controllers.drop(state=flag, controller_id=caller_id)
    ...
    def drop_waiters_registration(self, flag):
        if not self.controllers.state_exists(flag):
            for waiter in filter(lambda wtr: wtr.flag == flag, self.queue):
